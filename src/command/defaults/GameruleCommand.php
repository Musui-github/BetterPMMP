<?php

/*
 *
 *  ____            _        _   __  __ _                  __  __ ____
 * |  _ \ ___   ___| | _____| |_|  \/  (_)_ __   ___      |  \/  |  _ \
 * | |_) / _ \ / __| |/ / _ \ __| |\/| | | '_ \ / _ \_____| |\/| | |_) |
 * |  __/ (_) | (__|   <  __/ |_| |  | | | | | |  __/_____| |  | |  __/
 * |_|   \___/ \___|_|\_\___|\__|_|  |_|_|_| |_|\___|     |_|  |_|_|
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Lesser General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * @author PocketMine Team
 * @link http://www.pocketmine.net/
 *
 *
 */

declare(strict_types=1);

namespace pocketmine\command\defaults;

use pocketmine\command\CommandSender;
use pocketmine\command\utils\InvalidCommandSyntaxException;
use pocketmine\lang\KnownTranslationFactory;
use pocketmine\permission\DefaultPermissionNames;
use pocketmine\player\Player;
use pocketmine\world\gamerule\GameruleIds;
use function count;
use function strtolower;

class GameruleCommand extends VanillaCommand{

	public function __construct(){
		parent::__construct(
			"gamerule",
			KnownTranslationFactory::pocketmine_command_gamerule_description(),
			KnownTranslationFactory::command_gamerule_usage(),
		);
		$this->setPermission(DefaultPermissionNames::COMMAND_GAMERULE);
	}

	/**
	 * @param CommandSender $sender
	 * @param string        $commandLabel
	 * @param array         $args
	 *
	 * @return void
	 */
	public function execute(CommandSender $sender, string $commandLabel, array $args) : void{
		if(!$this->testPermission($sender)){
			return;
		}

		if(count($args) < 2){
			throw new InvalidCommandSyntaxException();
		}

		$world = $sender instanceof Player ? $sender->getWorld() : $sender->getServer()->getWorldManager()->getDefaultWorld();
		$ruleName = strtolower($args[0]);
		if(!GameruleIds::validGamerule($ruleName)){
			$sender->sendMessage(KnownTranslationFactory::pocketmine_command_gamerule_invalid($ruleName));
			return;
		}

		$world->getGameruleManager()->setGamerule($ruleName, ($gameruleValue = match (GameruleIds::getType($ruleName)) {
			GameruleIds::TYPE_BOOL => boolval($args[1]),
			GameruleIds::TYPE_INT => intval($args[1]),
		}));
		$sender->sendMessage(KnownTranslationFactory::pocketmine_command_gamerule_success($ruleName, "{$gameruleValue}"));
	}
}
